...
fram <- read.csv(file.choose(),sep="")
View(fram)

attach(fram)

err <- sbp2 - (sbp2+sbp3)/2
sderr <- sd(err)

con0 <- subset(fram,firstchd == 0)
cas0 <- subset(fram,firstchd == 1)
...
#a <- max(quantile(z0,0.25),quantile(z1,0.25))
#b <- min(quantile(z0,0.75),quantile(z1,0.75))
a <- mean(z0)
b <- mean(z1)
